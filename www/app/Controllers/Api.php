...
class Api extends BaseController{
    ...
    function cardList($babyId){
        $this->moddb->chkDataApi('babys',array('babyId'=>$babyId),'寶寶不存在');
        $data = $this->cards->getList($babyId);
    ...
    function cardAdd(){
        ...
        $data['checkData'] = $this->cards->cardCheckDataGrid($data['row'],$data['col']);
        if($this->cards->addOnce($data)){
            $this->api->show('200');
        }else{
            $this->api->show('500');
        }
    ...
    function stampAdd(){
        ...
        $baby = $this->moddb->chkDataApi('babys',array('babyId'=>$card['babyId']),'寶寶不存在');
        ...
        if($cardData[$data['cardNum']]['status'] == 0){
            ...
            $this->cards->updateOnce($data['cardId'],$setData);
    ...
    function stampUse(){
        ...
        $cardData = json_decode($card['checkData'],true);
        if($cardData[$data['cardNum']]['status'] == 1){
            // 設定集點卡
            $cardData[$data['cardNum']]['status'] = 2;
            $setData['checkData'] = json_encode($cardData);
            $setData['updateDatetime'] = date("Y-m-d H:i:s");
            $this->cards->updateOnce($data['cardId'],$setData);
            ...
            $data['closeStamp'] = $data['initStamp'] + $data['offsetStamp'];
            if($data['notice'] == ''){
                $data['notice'] = $user['displayName'].'使用掉一個章';
            }
            unset($data['cardNum']);
            $this->cards->setStampLog($data);
            // 回填總數
    ...
    function stampLog($cardId){
        $card = $this->moddb->chkDataApi('card',array('cardId'=>$cardId),'卡片不存在');
        $log = $this->cards->getStampLog($cardId);
        $this->api->show('200',$log);
    }
}